'use client'
import { Button, Card, Modal,Spin,Empty } from 'antd'
...
function Page() {
  ...
  useEffect(() => {
    if (userDetails?.token) {
      fetchApi({ URI: `customers/service/request/getBy/${userDetails?.userId}`, API_TOKEN: userDetails?.token })
        .then((res) => {
  ...
  return (
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <Empty description="Currently no service requestd!" />
      ) : (
        requests?.map((item, idx) => (
          <Card key={idx}>
            ...
            <div className='grid mt-4'>
              ...
              <h3 className='font-semibold truncate'>{item?.Subject}
              </h3>
              <p className='truncate'>{item?.Description}</p>
              <p className='text-xs'>Dated: {item?.createdAt}</p>
            </div>
          </Card>
        ))
      )}
