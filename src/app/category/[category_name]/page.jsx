...
function Page() {
  ...
  useEffect(() => {
    setLoading(true)
    fetchApi({ URI: `public/products?${filter({ params: searchparams })}&populate=customLabel&filters[category][slug][$eq]=${params?.category_name}&populate=category&sort=createdAt:Desc&pagination[page]=${pagination?.pageNo || 1}&pagination[pageSize]=${pagination?.pageSize || 20}` })
      .then(res => {
        setProducts(res)
