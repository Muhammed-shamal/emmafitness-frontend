...
function DeliveryAddress({ address = [], selected, reload }) {
  ...
  return (
    <div className='bg-gray-100 border border-gray-200 p-2 md:p-4 space-y-4'>
      <Modal open={popup} footer={false} onCancel={popupCloseHandle}><CreateAddress close={popupCloseHandle}/></Modal>
      <div className='flex flex-row justify-between text-sm font-semibold'>
        Delivery Address
        <div className='text-blue-600 cursor-pointer' onClick={() => setPopup(true)}> Create New </div>
      </div>


      <Radio.Group className='grid grid-cols-1 md:grid-cols-3  overflow-y-auto gap-4 rounded-none' buttonStyle='outline' onChange={selected}>
        {
          address?.map(({ id, attributes }) => (

            <Radio.Button key={id} className=' bg-gray-200 rounded-none h-32' value={id}>
